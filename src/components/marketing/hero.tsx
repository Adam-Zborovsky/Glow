...
export function Hero() {
  return (
    <section className="pt-32 pb-24 px-8 lg:px-16 max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center overflow-visible">
      <div className="flex flex-col items-start gap-8 z-10">
        <Badge variant="secondary" className="px-4 py-1.5 rounded-full bg-violet-50 text-violet-600 border-violet-100 flex gap-2 items-center">
        ...
        <div className="mt-16 flex flex-col gap-6 w-full">
          ...
          <div className="flex items-center gap-10 opacity-60">
            ...
            <div className="flex -space-x-3 ml-4">
              ...
              <div className="w-10 h-10 rounded-full border-2 border-white bg-violet-600 text-white flex items-center justify-center text-[10px] font-bold">
                +50k
              </div>
            </div>
          </div>
      ...
      <div className="relative flex justify-center items-center overflow-visible">
        ...
        {/* iPhone Mockup */}
        <div className="iphone-frame relative w-[320px] h-[640px] bg-slate-900 rounded-[3rem] p-3 border-[8px] border-slate-800 overflow-hidden z-20">
          <div className="w-full h-full hero-gradient rounded-[2rem] relative overflow-hidden flex flex-col p-6 items-center">
            <div className="mt-12 flex flex-col items-center gap-4">
              <div className="w-24 h-24 rounded-full p-1 bg-white/30 backdrop-blur-sm shadow-xl overflow-hidden">
              ...
              <div className="text-center text-white">
                ...
                <p className="text-xs opacity-80 font-medium tracking-wide">Designer & Creator</p>
              </div>
            </div>
            
            <div className="mt-10 w-full flex flex-col gap-3">
              {["My Portfolio", "Latest YouTube Video", "Book a Call", "Shop My Prints"].map((link) => (
                <div key={link} className="w-full py-4 bg-white/20 backdrop-blur-md border border-white/30 rounded-2xl flex items-center justify-center text-white font-semibold text-sm shadow-sm">
                  {link}
                </div>
              ))}
            </div>
        ...
        <div className="absolute -right-12 top-[15%] glass-card p-4 rounded-2xl shadow-2xl flex items-center gap-3 z-30 animate-float" style={{ animationDelay: '0s' }}>
          <div className="w-10 h-10 rounded-full bg-cyan-400 flex items-center justify-center text-white shadow-inner">
            <UserPlus className="w-5 h-5" />
          </div>
          <div>
            <p className="text-[10px] text-cyan-600 font-extrabold uppercase tracking-widest">Growth</p>
            <p className="text-xs font-bold text-slate-800">New Subscriber!</p>
          </div>
        </div>
        
        <div className="absolute -left-16 top-[40%] glass-card p-4 rounded-2xl shadow-2xl flex items-center gap-3 z-30 animate-float" style={{ animationDelay: '1.5s' }}>
          <div className="w-10 h-10 rounded-full bg-emerald-500 flex items-center justify-center text-white shadow-inner">
            <CircleDollarSign className="w-5 h-5" />
          </div>
          <div>
            <p className="text-[10px] text-emerald-600 font-extrabold uppercase tracking-widest">Earnings</p>
            <p className="text-xs font-bold text-slate-800">Payment received $50</p>
          </div>
        </div>
        
        <div className="absolute -right-8 bottom-[20%] glass-card p-4 rounded-2xl shadow-2xl flex items-center gap-3 z-30 animate-float" style={{ animationDelay: '3s' }}>
          <div className="w-10 h-10 rounded-full bg-blue-500 flex items-center justify-center text-white shadow-inner">
            <Eye className="w-5 h-5" />
          </div>
          <div>
            <p className="text-[10px] text-blue-600 font-extrabold uppercase tracking-widest">Traffic</p>
            <p className="text-xs font-bold text-slate-800">+1,240 profile views</p>
          </div>
        </div>
      </div>
    </section>
